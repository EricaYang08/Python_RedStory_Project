...
def Char5(bag, clue):
    if not clue:
        with open("chap_5_noclues.txt", encoding='utf-8') as f:
            print(f.read())
        sys.exit()
    else:
        with open("chap_5_withclues.txt", encoding='utf-8') as f:
            print(f.read())
        choice = input("Type A for Outwit and B for Boldly: ")
        if choice.lower() == "a":
            if bag['knife'] > 0:
                with open("chap_5_withclues_outwit_knife.txt", encoding='utf-8') as f:
                    print(f.read())
            else:
                with open("chap_5_withclues_outwit_noknife.txt", encoding='utf-8') as f:
                    print(f.read())
        elif choice.lower() == "b":
            if bag['knife'] > 0:
                with open("chap_5_withclues_confront_knife.txt", encoding='utf-8') as f:
                    print(f.read())
            else:
                with open("chap_5_withclues_confront_noknife.txt", encoding='utf-8') as f:
                    print(f.read())
